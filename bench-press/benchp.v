...
fn main() {
   ...
   */
  thing := get_default_lua_conf()or{return}
  println("hello ${thing}")
	 return
}
...
fn get_default_lua_conf() !string {
	lua_code := os.read_file(default_lua_file_path) or {
		return error('failed to read config file.')
	}
	return lua_code.replace('$1', get_benchp_conf_dir_path()).replace('$2', get_prg_name()).split('\n').join('\\n')
}
...
fn create_benchp_dir_and_files(tac_y bool) !bool {
	...
	if u1_ok {
		...
		lua_code := get_default_lua_conf() or {
			...
			return error('# faield to read default file, copy it from here: ${benchfile_lua_path}. you may re-try config creation process or check existance of benchfile.lua in provided path.')
		}
		files := {
			'.gitignore':    '*\n!.gitignore'
